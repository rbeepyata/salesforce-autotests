package com.epam.automation.core.element;
import com.epam.automation.exception.TestFailedException;

/**
 * The {@code Image} class is a concrete implementation of the base class {@link ElementBase}, and
 * it models an image in a web page.
 * The class {@code Image} includes only methods that are semantically relevant for an image in a web page.
 * 
 * <p> Epam Automation - Core </p>
 * @author Raghunandan Beepyata
 * 
 */
public class Image extends ElementBase {
	
	public Image(String elementName, Locator elementLocator){
		this.elementName = elementName;
		this.elementLocator = elementLocator;
	}
	
	public void click() throws TestFailedException{
		actions.click(elementLocator, elementName);
	}
	
	@Override
	public Image highlight() {
		actions.highlight(elementLocator, elementName);
		return this;
	}

	@Override
	public Image scrollIntoView() {
		actions.scrollIntoView(elementLocator, elementName);
		return this;
	}
	
}
